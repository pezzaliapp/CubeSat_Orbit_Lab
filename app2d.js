/* Canvas 2D fallback (trimmed v3) */
(function(){
'use strict'; var canvas=document.getElementById('view2d'); var ctx=canvas.getContext('2d');
var W=canvas.width,H=canvas.height,cx=W/2,cy=H/2; var running=false,t=0,timescale=5,trailMax=800,trail=[],logSamples=[];
var R_EARTH=6371e3,MU=3.986004418e14,RHO0=1.225,Hs=8500;
var perigeeAlt=400e3,apogeeAlt=400e3,inclDeg=51,raanDeg=0,argpDeg=0,m0Deg=0; var useDrag=false,CdA_over_m=0.02;
var showAxes=true,showOrbit=true,showAtmo=true,showShadow=true;
var cam={r:2.8*R_EARTH,theta:-0.8,phi:0.9,fov:900,target:{x:0,y:0,z:0}}; var sunTheta=0;
...
function w2s(p){var cp=sph(cam.r,cam.theta,cam.phi),f=nrm(sub(cam.target,cp)),r=nrm(cr(f,v3(0,1,0))),u=nrm(cr(r,f));var rel=sub(p,cp);
 var x=dot(rel,r),y=dot(rel,u),z=dot(rel,f),s=cam.fov/Math.max(1e-6,(cam.fov+z));return {x:cx+x*s,y:cy-y*s,z:z}}
function el2sv(a,e,i,raan,argp,M0,t){var n=Math.sqrt(MU/Math.pow(a,3)),M=(M0+n*t)%(2*Math.PI),E=M;for(var k=0;k<10;k++){var f=E-e*Math.sin(E)-M,fp=1-e*Math.cos(E);E=E-f/fp}
 ...
 var R11=cO*co - sO*so*ci, R12=-cO*so - sO*co*ci, R21=sO*co + cO*so*ci, R22=-sO*so + cO*co*ci, R31=so*si, R32=co*si, R33=ci;
...
function drawEarth(){var p=w2s(v3(0,0,0)),R=projR(R_EARTH);ctx.fillStyle="#0a1d47";ctx.beginPath();ctx.arc(p.x,p.y,R,0,6.283);ctx.fill();
 if(showAtmo){var Ra=R*1.05;var g2=ctx.createRadialGradient(p.x,p.y,R*.95,p.x,p.y,Ra);g2.addColorStop(0,"rgba(120,200,255,.05)");g2.addColorStop(1,"rgba(120,200,255,0)");
...
var elPer=perigee,elApo=apogee,elIncl=incl,elRAAN=raan,elArgp=argp,elM0=m0,elTS=timescale,elTrail=trail,elPerV=perigeeVal,elApoV=apogeeVal,elInclV=inclVal,elRAANV=raanVal,elArgpV=argpVal,elM0V=m0Val,elTSV=timescaleVal,elTrailV=trailVal,elScenario=scenario,elDrag=drag,elCdA=cda,elCdAV=cdaVal;
